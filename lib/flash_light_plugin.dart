// You have generated a new plugin project without specifying the `--platforms`
// flag. A plugin project with no platform support was generated. To add a
// platform, run `flutter create -t plugin --platforms <platforms> .` under the
// same directory. You can also find a detailed instruction on how to add
// platforms in the `pubspec.yaml` at
// https://flutter.dev/to/pubspec-plugin-platforms.

import 'package:flutter/services.dart';

import 'flash_light_plugin_platform_interface.dart';

class FlashLightPlugin {
  static const flashLightChannel = MethodChannel('flash_light_plugin');

  Future<String?> getPlatformVersion() {
    return FlashLightPluginPlatform.instance.getPlatformVersion();
  }

  static Future turnOn() async {
    await flashLightChannel.invokeMethod('turnOnFlashLight');
  }

  static Future turnOff() async {
    await flashLightChannel.invokeMethod('turnOffFlashLight');
  }
}
